...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="
		https://www.wireshark.org/download/src/all-versions/${P}.tar.xz
...
IUSE+=" sdjournal +stratoshark test +text2pcap +tshark +udpdump wifi xxhash zlib +zstd"
...
RDEPEND="
	...
	stratoshark? (
		>=dev-libs/falcosecurity-libs-0.17.1:=
		dev-libs/uthash
		>=net-libs/libssh-0.6:=
	)
...
PATCHES=( "${FILESDIR}/4.4.6-lto.patch" )
	...
	if [[ ${PV} == *9999* ]]; then
		...
		if use verify-sig; then
			...
			verify-sig_verify_signed_checksums \
				${P}-signatures.txt openssl-dgst ${P}.tar.xz
			cd "${WORKDIR}" || die
		fi
...
src_configure() {
	...
	mycmakeargs=(
		...
		-DBUILD_stratoshark=$(usex stratoshark)
		-DBUILD_strato=$(usex stratoshark)
		-DBUILD_dumpcalls=$(usex stratoshark)
		-DBUILD_falcodump=$(usex stratoshark)
		-DBUILD_sshdig=$(usex stratoshark)
		-DUSE_qt6=$(usex gui)
		-DENABLE_WERROR=OFF
...
src_test() {
	...
	epytest --disable-capture --skip-missing-programs=all \
...
src_install() {
	...
	if use gui; then
		...
		for s in 16 32 48 64 128 256 512 1024; do
			[[ -f resources/icons/wsicon${s}.png ]] || continue
			insinto /usr/share/icons/hicolor/${s}x${s}/apps
			newins resources/icons/wsicon${s}.png wireshark.png
		done
		for s in 16 24 32 48 64 128 256; do
			[[ -f resources/icons/WiresharkDoc-${s}.png ]] || continue
...
pkg_postinst() {
	...
	ewarn "To capture network traffic as a normal user, add the user to the pcap group."
}
